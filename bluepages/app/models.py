...
REGION_TYPE_CHOICES = [("N", "Near Shore"), ("M", "Mid Water"), ("O", "Offshore")]
...
class Region(models.Model):
    ...
    geometry = GeometryField(
        srid=3857, null=True, blank=True, verbose_name="Region Boundary", default=None
    )

    depth_type = models.CharField(
        max_length=1,
        default="N",
        choices=REGION_TYPE_CHOICES,
        verbose_name="Region Depth Type",
    )
    id_num = models.CharField(
        max_length=4, null=True, blank=True, default=None, verbose_name="Region Number"
    )
    ...
    def __str__(self):
        states = ",".join(
            [f"st:{x.postal_code}" for x in self.states.all().order_by("postal_code")]
        )
...
class Entity(models.Model):
    ...
    def to_dict(self, include_contacts=False, flat=False):
        ...
        if include_contacts and self.allow_show_contacts() and not flat:
            out_dict["contacts"] = [
                contact.to_dict(include_records=False) for contact in self.contacts
            ]
        ...
        if flat or self.parent == self:
            ...
            out_dict["parent"] = (
                self.parent.to_dict(include_contacts=False, flat=flat)
                if self.parent
                else None
            )
...
class RecordBase(models.Model):
    ...
    def general_regions(self):
        ...
        depth_lookup = {"N": "near shore", "M": "mid", "O": "offshore"}
        region_map = {}
            ...
            if key in state_list and key not in trifectas:
                ...
                if len(state_depth_list) > 0:
                    general_region_list.append(
                        "{} {} waters".format(key, " and ".join(state_depth_list))
                    )
...
class Record(RecordBase):
    ...
    def to_dict(
        self, include_contact=False, include_regions=True, include_geometry=False
    ):
        out_dict = {
            "id": self.pk,
            "name": str(self),
        ...
        if include_regions:
            out_dict["regions"] = [
                region.to_dict(include_geometry=include_geometry)
                for region in self.regions.all()
            ]
...
class ContactBase(models.Model):
    ...
    first_name = models.CharField(
        max_length=254, blank=True, default="", verbose_name="First/Given Name"
    )
    last_name = models.CharField(max_length=254, verbose_name="Last/Family Name")
    middle_name = models.CharField(
        max_length=254, blank=True, default="", verbose_name="Middle Name(s)/Initial(s)"
    )
    ...
    job_title = models.CharField(
        max_length=254, blank=True, default="", verbose_name="Job Title/Position"
    )
    expertise = models.CharField(
        max_length=254, blank=True, default="", verbose_name="Area(s) of expertise"
    )
    email = models.EmailField(
        max_length=254,
        blank=True,
    ...
    phone = PhoneField(
        blank=True, null=True, default=None, verbose_name="Work/Desk phone no."
    )
    mobile_phone = PhoneField(
        blank=True, null=True, default=None, verbose_name="Mobile phone no."
    )
    office_phone = PhoneField(
        blank=True, null=True, default=None, verbose_name="Department/General phone no."
    )
    fax = PhoneField(
        blank=True,
        null=True,
    ...
    show_on_entity_page = models.BooleanField(
        ...
        + "<span>Filtered: Contact only disoverable via region/topic search.</span><br />"
...
class Contact(ContactBase):
    ...
    def to_dict(
        ...
        if include_records:
            records = [
                record.to_dict(
                    include_regions=include_regions, include_geometry=include_geometry
                )
                for record in self.record_set.all().order_by("topic__name")
            ]
            if flat:
                topics = []
                region_ids = []
                ...
                regions = [
                    x.name
                    for x in Region.objects.filter(id__in=region_ids).order_by("name")
                ]
...
class ContactSuggestion(ContactBase):
    user = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, verbose_name="User proposing this change"
    )
    contact = models.ForeignKey(
        "Contact",
        blank=True,
    ...
    other_entity_name = models.CharField(
        max_length=254,
        blank=True,
        default="",
        verbose_name="Other Entity Name",
        help_text="If contact belongs to an unlisted entity, name it here.",
    )
    sub_entity_name = models.CharField(
        max_length=254,
        blank=True,
        default="",
        verbose_name="Division/Sub-entity",
        help_text="If contact belongs to a specific division within their organization, please specify it here.",
    )
    status = models.CharField(
        max_length=20,
        default="Pending",
        choices=SUGGESTION_STATUS_CHOICES,
        verbose_name="Suggestion status",
        help_text="Has suggestion been approved or declined?",
    )
    description = models.TextField(
        null=True,
        blank=True,
        default=None,
        verbose_name="Describe the change you are suggesting (if you are updating an existing contact)",
        help_text="If you are updating an existing contact, what specific changes are you trying to propose in this form?",
    )
    ...
    def clean(self):
        if not self.pk and self.contact and self.status == "Pending":
            matches = ContactSuggestion.objects.filter(
                contact=self.contact, user=self.user, status=self.status
            )
            if matches.count() > 0:
                raise ValidationError(
                    "You already have a suggested edit pending for this contact. Please edit your existing suggestion."
                )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                condition=(models.Q(status="Pending") & ~models.Q(contact=None)),
...
class RecordSuggestion(RecordBase):
    contact_suggestion = models.ForeignKey(
        "ContactSuggestion", on_delete=models.CASCADE
    )
    user = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, verbose_name="User proposing this change"
    )
    ...
    def clean(self):
        try:
            if (
                not self.pk
                and self.contact_suggestion.status == "Pending"
                and self.status == "Pending"
            ):
                matches = RecordSuggestion.objects.filter(
                ...
                if matches.count() > 0:
                    raise ValidationError(
                        "You already have a suggested edit pending for this contact for this topic. Please edit your existing suggestion."
                    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["user", "topic", "contact_suggestion", "status"],
