...
def exportCSVList(request):
    ...
    try:
        ...
        csv_file = tempfile.NamedTemporaryFile(
            prefix=prefix, suffix=".csv", delete=False
        )
        ...
        with open(csv_file.name, "w") as csv_contents:
            ...
            writer.writeheader()
            for contact in contacts.order_by(
                "last_name",
                "first_name",
                "middle_name",
                "post_title",
...
def filterContacts(filters={}, format="datatable"):
    ...
    if (
        ("entities" not in filters.keys() or len(filters["entities"]) == 0)
        and ("topics" not in filters.keys() or len(filters["topics"]) == 0)
        and ("map_regions" not in filters.keys() or len(filters["map_regions"]) == 0)
    ):
    ...
    if "topics" in filters.keys() and len(filters["topics"]) > 0:
        records = Record.objects.filter(
            contact__in=contacts, topic__pk__in=filters["topics"]
        )
...
class changePassword(View):
    ...
    def post(self, request):
        ...
        if password_form.is_valid():
            password_form.save()
            context["message"] = (
                'Your password has been updated. You will now be logged out. Please log in again with your new password. <span id="password-reset-success"></span>'
            )
            context["form"] = ""
    ...
    final_regions = [
        {
            "name": "Washington",
            "id": "WA",
            "count": regions_dict["Washington"]["count"],
        },
        {"name": "Oregon", "id": "OR", "count": regions_dict["Oregon"]["count"]},
        {
            "name": "California",
            "id": "CA",
            "count": regions_dict["California"]["count"],
        },
        {"name": "Offshore", "id": "OS", "count": regions_dict["Offshore"]["count"]},
        {
            "name": "Middle-depth",
            "id": "MD",
            "count": regions_dict["Middle-depth"]["count"],
        },
...
def getSuggestionMenu(request):
    user_suggestions = [
        ...
        ).order_by("last_name", "first_name", "date_modified", "date_created")
    ]
    for contact_suggestion in (
        ContactSuggestion.objects.filter(user=request.user)
        ...
        if contact_id:
            ...
            context = {
                "contact": {
                    ...
                    "records": contact_suggestion.recordsuggestion_set.all().order_by(
                        "topic"
                    ),
                    "description": contact_suggestion.description,
                    "status": contact_suggestion.status,
...
def deleteSuggestedContact(request, contact_id=None):
    ...
    for match in contact_suggestion_matches:
        match.delete()
    return JsonResponse(
        {"status": 200, "success": True, "message": "Suggestion deleted."}
    )
...
def contactSuggestionForm(request, contact_id=None):
    ...
    if contact_id:
        ...
        contact_suggestion_record_matches = ContactSuggestion.objects.filter(
            pk=contact_id, user=request.user
        )
    ...
    if request.method == "POST":
        ...
        if contact_suggestion_record:
            ...
            contact_form = ContactSuggestionForm(
                initial={"user": request.user, "status": "Pending"}
            )
    context = {"contact_form": contact_form, "action": action, "contact_id": contact_id}
...
def recordSuggestionForm(request, contact_id, record_id=None):
    ...
    if request.method == "POST":
        ...
        if record_suggestion:
            ...
            record_form = RecordSuggestionForm(
                initial={
                    "user": request.user,
                    "status": "Pending",
...
def getContactJsonLd(request, contact, render=False):
    ...
    for record in contact.record_set.all():
        doc["knowsAbout"].append(
            {"@type": "Text", "description": str(record.topic)},
            # {
            # "@type": "URL",
...
def entityDetailHTML(request, id):
    ...
    return render(
        request,
        "entity_detail_html.html",
        {"entity": entity, "JSON_LD": json_ld, "embedded": False},
    )
...
def entityDetailEmbedded(request, id):
    ...
    return render(
        request,
        "entity_detail_embedded_wrapper.html",
        {"entity": entity, "JSON_LD": json_ld, "embedded": True},
    )
...
def buildReviewRow(
    ...
    try:
        cells.append(
            {
                "value": contact_form.fields[contact_field].get_bound_field(
    ...
    if contact:
        if hasattr(contact, field):
            cells.insert(1, {"value": getattr(contact, field)})
        else:
...
def adminSuggestionReviewMenu(request, suggestion_id):
    ...
    try:
            ...
            if contact_form.is_valid():
                ...
                try:
                    ...
                    for record_suggestion in suggestion.recordsuggestion_set.all():
                        record_approve_name = "approve-record-suggestion-{}".format(
                            record_suggestion.pk
                        )
                        if (
                            record_approve_name in request.POST.keys()
                            and request.POST[record_approve_name] == "on"
                        ):
                        ...
                        try:
                            ...
                            (record, rec_create_status) = Record.objects.get_or_create(
                                contact=contact, topic=record_suggestion.topic
                            )
                            record.regions.set(record_suggestion.regions.all())
                            record.save()
                            message = "Record Suggestion '{}' {}.".format(
                                record_suggestion, record_suggestion.status
                            )
                            messages.add_message(
                                request,
                                messages.SUCCESS,
                ...
                return HttpResponseRedirect(
                    "/admin/app/contactsuggestion/{}/change/".format(suggestion_id)
                )
        ...
        if contact:
            header_cells.insert(2, {"value": "Current Record"})
        rows.append({"element": "th", "cells": header_cells})
        ...
        rows.append(
            buildReviewRow(suggestion, contact, contact_form, "other_entity_name")
        )
        rows.append(
            buildReviewRow(suggestion, contact, contact_form, "sub_entity_name")
        )
        ...
        rows.append(
            buildReviewRow(
                suggestion, contact, contact_form, "preferred_contact_method"
            )
        )
        rows.append(
            buildReviewRow(suggestion, contact, contact_form, "show_on_entity_page")
        )
        ...
        for record_suggestion in suggestion.recordsuggestion_set.all().order_by(
            "topic"
        ):
...
def adminSuggestionRejection(request, suggestion_id):
    ...
    try:
        ...
        try:
            ...
            suggestion.save()
            message = "Contact suggestion '{}' successfully declined.".format(
                str(suggestion)
            )
            messages.add_message(
                request,
    ...
    for region in regions:
        ...
        properties = {
            ...
            "states": ",".join(
                [
                    f"{x.postal_code}"
                    for x in region.states.all().order_by("postal_code")
                ]
            ),
        }
        region_json = {
